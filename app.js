...
function displayAttendanceRecords() {
  ...
  const today = new Date().toLocaleDateString();
  let dataToUse = attendanceData;
  if (currentUser.role === "student") {
    dataToUse = attendanceData.filter(a => a.rollNumber === currentUser.rollNumber);
  }
  const todayRecords = dataToUse.filter(a => a.date === today);
...
function displayRecentActivity() {
  ...
  const recent = dataToUse.slice(0, 5);
...
function displayDepartmentBreakdown() {
  ...
  const card = container.closest(".dept-breakdown-card");
  if (currentUser.role === "student") {
...
function initializeStudentPanel() {
  ...
  // Export button
  const exportBtn = document.getElementById('sa-history-export-btn');
  if (exportBtn) {
...
async function checkTodayStatus() {
  ...
  const roll = currentUser.rollNumber || '';
  const today = new Date().toLocaleDateString();
  const container = document.getElementById('sa-today-status-mini');
  if (!container) return;
...
function initializeProfile() {
  ...
  const userDisplay = document.getElementById("profileNameDisplay");
  const roleDisplay = document.getElementById("profileRoleDisplay");
  const avatar = document.getElementById("profileAvatar");
  const toggleBtn = document.getElementById("toggleEditBtn");
  const updateBtn = document.getElementById("updateProfileBtn");
  const formInputs = document.querySelectorAll("#profileForm input:not([disabled])");

  if (nameInput) nameInput.value = currentUser.name || "";
  if (document.getElementById("profUsername")) document.getElementById("profUsername").value = currentUser.username || "";
  if (document.getElementById("profEmail")) document.getElementById("profEmail").value = currentUser.email || "";
  if (document.getElementById("profPhone")) document.getElementById("profPhone").value = currentUser.phone || "";
  
  if (userDisplay) userDisplay.textContent = currentUser.name || currentUser.username;
  if (roleDisplay) roleDisplay.textContent = (currentUser.role || "student").toUpperCase();

  // Avatar initial
  if (avatar) avatar.textContent = (currentUser.name || currentUser.username).charAt(0).toUpperCase();

  // Student specific fields
  const studentOnlyRows = document.querySelectorAll(".student-only");
  if (currentUser.role === "student") {
      studentOnlyRows.forEach(el => el.style.display = "flex");
      const rollInput = document.getElementById("profRollNumber");
      const deptInput = document.getElementById("profDept");
      if (rollInput) rollInput.value = currentUser.rollNumber || "";
      if (deptInput) deptInput.value = currentUser.department || "";
  } else {
      studentOnlyRows.forEach(el => el.style.display = "none");
  }
  ...
  const setEditMode = (editing) => {
      ...
      formInputs.forEach(input => {
          if (input.id !== "profUsername" && !input.readOnly && !input.disabled) {
              // Only toggle if it's not already restricted
          }
          if (input.id !== "profUsername" && input.id !== "profRollNumber" && input.id !== "profDept") {
              input.readOnly = !editing;
              input.style.background = editing ? "var(--card-bg)" : "rgba(0,0,0,0.02)";
          }
      });
      if (toggleBtn) toggleBtn.innerHTML = editing ? "✕ Cancel" : "✏️ Edit Details";
      if (updateBtn) updateBtn.parentNode.style.display = editing ? "block" : "none";
  };

  setEditMode(false);

  if (toggleBtn) {
      toggleBtn.onclick = () => setEditMode(!isEditing);
  }
